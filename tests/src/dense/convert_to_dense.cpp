...
TEST_P(ConvertToDenseTest, FromDense) {
    ...
    auto converted2 = tatami::convert_to_dense<int, std::size_t>(&mat, to_row, threads); // works for a different type.
    EXPECT_EQ(converted2->prefer_rows(), to_row);
    EXPECT_FALSE(converted2->is_sparse());

    auto buffer = sanisizer::create<std::vector<double> >(NC);
    sanisizer::as_size_type<std::vector<int> >(NC);
    std::vector<int> expected(NC), observed(NC);
    auto old = mat.dense_row();
    auto wrk2 = converted2->dense_row();

    for (int r = 0; r < NR; ++r) {
        auto ptr = old->fetch(r, buffer.data());
        std::copy_n(ptr, NC, expected.data());
        auto ptr2 = wrk2->fetch(static_cast<std::size_t>(r), observed.data());
        tatami::copy_n(ptr2, NC, observed.data());
