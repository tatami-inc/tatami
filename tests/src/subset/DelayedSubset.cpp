...
class SubsetFullAccessTest : public SubsetTest<std::tuple<size_t, bool, bool, bool, bool, tatami_test::TestAccessOrder, size_t> > {};

TEST_P(SubsetFullAccessTest, OnRow) {
    auto tparam = GetParam();
    auto sub = spawn_indices<size_t>(std::get<0>(tparam), NR, std::get<1>(tparam), std::get<2>(tparam));
    ...
    param.use_row = std::get<3>(tparam);
    param.use_oracle = std::get<4>(tparam);
    param.order = std::get<5>(tparam);
    param.jump = std::get<6>(tparam);

    tatami_test::test_full_access(param, dense_subbed.get(), ref.get());
    tatami_test::test_full_access(param, sparse_subbed.get(), ref.get());
    tatami_test::test_full_access(param, sparse_subbed2.get(), ref.get());
}
