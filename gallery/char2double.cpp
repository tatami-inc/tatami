...
int main(int argc, char** argv) {
     ...
     * contrast, if vals was a vector of doubles, the code below would print 0.)
     */
    std::vector<double> vbuffer(mat->nrow());
    std::vector<int> ibuffer(mat->nrow());
    auto range = mat->sparse_column(0, vbuffer.data(), ibuffer.data());
    std::cout << "Using buffer instead of underlying store: " << (range.value==vbuffer.data()) << std::endl;
