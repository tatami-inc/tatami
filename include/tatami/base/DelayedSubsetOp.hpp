...
public:
    sparse_range<T, IDX> sparse_row(size_t r, T* out_values, IDX* out_indices, size_t start, size_t end, workspace_ptr work=nullptr) const {
        if constexpr(MARGIN==1) {
    ...
    sparse_range<T, IDX> sparse_column(size_t c, T* out_values, IDX* out_indices, size_t start, size_t end, workspace_ptr work=nullptr) const {
        if constexpr(MARGIN==1) {
...
private:
    ...
    template<bool ROW>
    void subset_expanded(size_t r, T* buffer, size_t start, size_t end, workspace_ptr work) const {
        while (start < end) {
            auto original = start;
    ...
    template<bool ROW>
    size_t subset_sparse(size_t r, T* out_values, IDX* out_indices, size_t start, size_t end, workspace_ptr work) const {
        size_t total = 0;
        while (start < end) {
            auto original = start;
            auto previdx = indices[start];
