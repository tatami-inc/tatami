...
public:
     ...
     */
    DelayedSubsetSortedUnique(std::shared_ptr<const Matrix<Value_, Index_> > matrix, SubsetStorage_ subset, bool row, bool check = true) :
        my_matrix(std::move(matrix)), my_subset(std::move(subset)), my_row(row)
    {
        ...
        Index_ mapping_dim = my_row ? my_matrix->nrow() : my_matrix->ncol();
