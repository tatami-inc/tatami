...
void apply_sparse_running(size_t dim, size_t otherdim, const Matrix<T, IDX>* p, Factory& factory, int threads) {
    ...
    if constexpr(stats::has_sparse_running_parallel<Factory>::value) {
        ...
        TATAMI_CUSTOM_PARALLEL(dim, [&](size_t start, size_t end) -> void {
            ...
            if (start < end) {
                ...
                auto wrk = new_extractor<!ROW, true, T, IDX>(p, Options<Index_>(), start, len); // flipped around; remember, we're trying to get the preferred dimension.
    ...
    auto wrk = new_extractor<!ROW, true, T, IDX>(p); // flipped around; remember, we're trying to get the preferred dimension.
...
void apply_dense_running(size_t dim, size_t otherdim, const Matrix<T, IDX>* p, Factory& factory, int threads) {
    ...
    if constexpr(stats::has_dense_running_parallel<Factory>::value) {
        ...
        TATAMI_CUSTOM_PARALLEL(dim, [&](size_t start, size_t end) -> void {
            ...
            if (start < end) {
                ...
                auto wrk = new_extractor<!ROW, false, T, IDX>(p, Options<Index_>(), start, len); // flipped around, see above.
    ...
    std::vector<T> obuffer(dim);
    auto wrk = new_extractor<!ROW, false, T, IDX>(p); // flipped around, see above.
...
void apply_sparse_direct(size_t dim, size_t otherdim, const Matrix<T, IDX>* p, Factory& factory, int threads) {
    ...
    TATAMI_CUSTOM_PARALLEL(dim, [&](size_t start, size_t end) -> void {
        ...
        auto wrk = new_extractor<ROW, true, T, IDX>(p);
        ...
        for (size_t i = start; i < end; ++i) {
            ...
            if constexpr(do_copy) {
                auto range = wrk->fetch_copy(i, obuffer.data(), ibuffer.data());
...
#endif
        std::vector<T> obuffer(otherdim);
        auto wrk = new_extractor<ROW, false, T, IDX>(p);
...
#endif
            if constexpr(do_copy) {
                auto ptr = wrk->fetch_copy(i, obuffer.data());
